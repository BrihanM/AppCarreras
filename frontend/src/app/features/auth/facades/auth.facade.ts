...
import { LoginCredentials, RegisterPayload } from '@shared/interfaces';
...
export class AuthFacade {
  ...
  register(payload: RegisterPayload): void {
    ...
    this.authService
      ...
      .subscribe({
        next: () => {
          this.toastService.success('¡Cuenta creada! Bienvenido a StreetRaceX.');
          this.router.navigate([APP_ROUTES.DASHBOARD]);
        },
        error: (err) => {
          const msg = err.friendlyMessage ?? 'Error al crear cuenta.';
          this.error.set(msg);
          this.toastService.error(msg);
        },
      });
